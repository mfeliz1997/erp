...
export function CartDrawer({ isOpen, onClose, tenant }: { isOpen: boolean, onClose: () => void, tenant: any }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-primary text-primary-foreground">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-50">
              ...
              <button 
                onClick={handleClose}
                className="mt-4 px-8 py-3 bg-white border border-black font-bold text-xs   hover:bg-primary hover:text-primary-foreground transition-all rounded-xl"
              >
                Volver al catálogo
              </button>
            </div>
            ...
            cart.map((item) => (
              <div key={item.id} className="flex gap-4 border-b border-gray-100 pb-6 group">
                <div className="w-20 h-20 bg-gray-50 border border-gray-100 overflow-hidden flex-shrink-0 rounded-xl">
                  {item.image_url ? (
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-start">
                    <h4 className="font-bold  text-sm tracking-tight leading-tight">{item.name}</h4>
                    <button 
        ...
        {cart.length > 0 && (
          <div className="p-6 border-t border-gray-100 space-y-4 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
            <div className="flex justify-between items-end">
              <span className="text-gray-500  font-bold text-xs ">Subtotal</span>
              <span className="text-2xl font-semibold">RD$ {total.toLocaleString()}</span>
            </div>
            
            <button 
              onClick={handleSendOrder}
              className="w-full py-4 bg-primary text-primary-foreground font-bold   text-sm flex items-center justify-center gap-3 hover:bg-gray-900 transition-all active:scale-[0.98]"
            >
