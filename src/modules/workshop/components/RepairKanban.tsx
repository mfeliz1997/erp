        ...
        return (
          <div key={col.id} className="flex flex-col gap-4">
            <div className={`p-4 border-b-4 flex items-center justify-between ${col.color}`}>
              ...
              <Badge variant="outline" className="rounded-xl border-current font-bold">{columnOrders.length}</Badge>
            </div>
            ...
            <div className="flex-1 space-y-4">
              {columnOrders.map((order) => (
                <Card 
                  key={order.id} 
                  className="rounded-xl border border-gray-200 hover:shadow-sm rounded-xl transition-all cursor-grab active:cursor-grabbing bg-white"
                >
                  <CardHeader className="p-4 pb-2">
                    <div className="flex justify-between items-start mb-1">
                       <p className="text-xs font-semibold   text-zinc-400">#{order.id.substring(0, 6)}</p>
                       <Badge variant="outline" className="text-xs rounded-xl  font-bold px-1 py-0">{order.device_details?.brand}</Badge>
                    </div>
                  ...
                  <CardContent className="p-4 pt-2 space-y-3">
                    <p className="text-xs text-zinc-600 line-clamp-2 ">"{order.issue_description}"</p>
                    
                    <div className="space-y-1 pt-2 border-t border-zinc-100">
                       <div className="flex items-center gap-2 text-zinc-500">
                          <User className="w-3 h-3" />
                          <span className="text-xs font-bold  tracking-tight">{order.customers?.name}</span>
                       </div>
                       {order.assigned_to_profile && (
                          <div className="flex items-center gap-2 text-zinc-400">
                             <Wrench className="w-3 h-3" />
                             <span className="text-xs  font-medium">Téc: {order.assigned_to_profile.name}</span>
                          </div>
                       )}
                    </div>
                    ...
                    <div className="pt-2">
                      <select 
                        ...
                        className="w-full h-8 text-xs font-semibold   border border-gray-200 bg-zinc-50 rounded-xl px-2 focus:ring-0 outline-none"
                      >
                         {COLUMNS.map(c => (
